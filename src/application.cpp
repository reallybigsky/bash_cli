    ...
    while (!handler->isExit()) {
        try {
            ...
            if (after_parse.size() == 1) {
                lastReturnCode = handler->exec(after_parse.front(), vars, ios->getInput(), ios->getOutput());
            } else {
                i_file = tmpfile();
                for (size_t i = 0; i < after_parse.size(); ++i) {
                    o_file = tmpfile();
                    if (i == 0) {
                        lastReturnCode = handler->exec(after_parse[i], vars, ios->getInput(), o_file);
                    } else if (i == after_parse.size() - 1) {
                        lastReturnCode = handler->exec(after_parse[i], vars, i_file, ios->getOutput());
                        fclose(i_file);
                        fclose(o_file);
                        break;
                    } else {
                        lastReturnCode = handler->exec(after_parse[i], vars, i_file, o_file);
                    }
                    fclose(i_file);
                    rewind(o_file);
                    i_file = o_file;
