...
Возможные ошибки:
1. Переданные файлы не найдены -  Критическая ошибка `example_file: No such file or directory`.
2. Файл нельзя открыть для чтения - Обычная ошибка `example_file: Permission denied`.
...
Возможные ошибки: 
1. Не было передано ни одного файла - Критическая ошибка `No files were provided`
2. Переданные файлы не найдены - Обычная ошибка `example_file: No such file or directory`
3. Файл нельзя открыть для чтения - Обычная ошибка `example_file: Permission denied`
...
Возможные ошибки:
1. Количество аргументов больше 2 - Критическая ошибка `Wrong number of arguments`.
...
При вызове внешней программы будет создан синхронный дочерний процесс с помощью `boost::process::system`, которому будут переданы текущие потоки ввода-вывода команды, а также `boost::process::env`, в котором будут перечислены текущие переменные окружения.
...
1. Текущий `PATH` в виде `filesystem::path`.
2. Переменные окружения в виде `unordered_map<string, string>`, где ключ - имя переменной, значение - значение переменной.
...
- Ошибки на этапе выполнения команд делятся на две категории - критические и обычные. Критические ошибки прекращают выполнение пайплайна, выводят информацию об ошибке и приглашение пользователя на ввод `> `. При возникновении обычных ошибок пайплайн команд продолжает выполняться, при этом в консоль все равно выводится информация об ошибке.
